...
straight answer: what's malformed, what's lossy, what will misbehave at read
time. `tome {catalog,plugin,skill} lint <path>` reads a Tome artifact and
reports findings. It never fixes anything behind your back, and it never gives
up halfway through.
...
unparseable frontmatter becomes a finding against that file, and lint keeps
walking the rest of the tree. You always get the complete report and the
`Summary` line — never a stack trace half-way through, never a report that
silently stopped at the first bad file.
...
nothing changes (a fixpoint), reporting what it fixed and what still needs a
human. It will not touch judgment calls: an over-long description gets
flagged, not truncated.

Add `--dry-run` to see what `--autofix` *would* change without writing
anything. (Plain `lint` already writes nothing — `--dry-run` only means
something alongside `--autofix`.)
...
Lint's exit code is a verdict, not an accident:
...
Run this before tagging a release and unconverted residue never reaches your
readers. The full code table lives in the
[exit code reference](../reference/exit-codes.md).

## Pitfalls

- **Warnings don't fail by default.** A pipeline gating on lint's exit code
  passes a warning-riddled catalog unless you pass `--strict`.
...
- **`--autofix` won't exercise judgment.** Residual injections (`@path`,
